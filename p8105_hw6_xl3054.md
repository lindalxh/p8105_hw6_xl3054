...
This plot shows the adjusted odds ratios for solving homicides involving
make versus female victims across major US cities. Most estimates are
around or below 1, and many confidence intervals cross 1, indicating
that in most cities male-victim homicides are solved slightly lower
rates than female-victim homicides, and there is not significance
difference in the odds of solving homicides. Cities like New York, NY,
encounter significant lower odds of solving homicides for male victims
...
The bootstrap distribution of $\hat{r^2}$ is smooth and almost
symmetric, centered around 0.915. This means that the fitted model
...
samples. In contrast, the bootstrap distribution of the ratio
$\frac{\hat{\beta_1}}{\hat{\beta_2}}$ is extremely skewed. Most values
are around zero, and a very long tail exntends to very large numbers.
This suggests that ${\hat{\beta_2}}$ is often very close to zero in
bootstrap samples, causing the ratio to explode and making the statistic
highly unstable. The coefficient ratio is strongly influenced by
sampling noise.
...
|:--------------|-----------:|----------:|
| r^2           |     0.8933 |    0.9275 |
| beta1 / beta2 | -5431.7722 | 4837.4254 |
...
The residuals appear roughly centered around zero. A few extreme
residuals indicate potential outliers that the model does not capture
well.
...
Comparing the mean RMSE of three models, the proposed model with
predictors `babysex`, `bhead`, `blength`, `gaweeks`, `wtgain`,
`mheight`, `momage`, `mrace`, and `ppbmi` has the lowest average RMSE
(275.5092), indicating the best predictive performance among the three.
The interaction model performs slightly worse (average RMSE = 288.3849),
while the main-effects model has the highest average RMSE (330.7093).
